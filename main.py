...
def generate_recommendations(final_risk):
    if final_risk >= 30:
        return "🔴 Very High Risk Management: High-intensity statin, PCSK9 inhibitor, SBP <130 mmHg."
    elif final_risk >= 20:
        return "🟠 High Risk Management: Moderate-intensity statin, SBP <130 mmHg."
    else:
        return "🟢 Moderate Risk Management: Lifestyle adherence, annual reassessment."
...
if baseline_risk:
    ...
    target_ldl = st.slider("LDL-C Target (mmol/L)", min_value=0.5, max_value=3.0, value=1.4, step=0.1)
